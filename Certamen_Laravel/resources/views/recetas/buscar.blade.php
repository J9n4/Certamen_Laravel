...
<div>
    <div class="bg-white rounded-2xl shadow-lg p-6 mb-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-4">
            Resultados: "<span class="text-blue-600">{{ $query }}</span>"
        </h1>
        ...
        <form action="{{ route('recetas.buscar') }}" method="GET" class="mb-4">
            <div class="flex gap-2">
                ...
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition">Buscar</button>
            </div>
        </form>
        ...
        @if(count($resultados) == 0)
            <div class="text-center py-8">
                ...
                <a href="{{ route('recetas.index') }}" class="inline-block mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg transition">Volver a todas</a>
            </div>
        @else
