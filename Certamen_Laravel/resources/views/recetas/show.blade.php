    ...
    <div class="bg-white rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-pink-600 p-8 text-white">
            ...
            <div class="flex flex-wrap gap-3">
                ...
                <span class="inline-block bg-white bg-opacity-20 px-4 py-2 rounded-full text-sm font-bold">⏱️ {{ $receta['tiempo'] }} min</span>
            </div>
        </div>
        ...
        <div class="p-8">
            <!-- Descripción -->
            <section class="mb-10">
                <h2 class="text-3xl font-bold text-gray-800 mb-4 border-b-4 border-purple-600 pb-3">Descripción</h2>
                <p class="text-lg text-gray-700">{{ $receta['descripcion'] }}</p>
            </section>

            <!-- Ingredientes -->
            <section class="mb-10">
                <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-4 border-purple-600 pb-3">Ingredientes</h2>
                <div class="space-y-3">
                    @foreach($receta['ingredientes'] as $ingrediente)
                        <div class="flex items-center gap-3 p-4 bg-green-50 rounded-lg border-l-4 border-green-500">
                            <span class="flex-shrink-0 w-8 h-8 bg-green-500 text-white rounded-full flex items-center justify-center">✓</span>
            ...
            <section class="mb-10">
                <h2 class="text-3xl font-bold text-gray-800 mb-6 border-b-4 border-purple-600 pb-3">Pasos de Preparación</h2>
                <div class="space-y-4">
                    @foreach($receta['pasos'] as $index => $paso)
                        <div class="flex gap-6 p-6 bg-blue-50 rounded-lg border-l-4 border-blue-500">
                            <div class="flex-shrink-0 w-12 h-12 bg-blue-500 text-white rounded-full flex items-center justify-center font-bold text-lg">{{ $index + 1 }}</div>
        ...
        <div class="bg-gray-50 px-8 py-6 border-t">
            <a href="{{ route('recetas.index') }}" class="inline-block bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-lg transition">Volver a recetas</a>
        </div>
    </div>
</div>
